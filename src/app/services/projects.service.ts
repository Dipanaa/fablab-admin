...
import {
  HitoProyecto,
  ProjectsResponse,
} from '../utils/responses-interfaces/projectsResponse';
import {
  catchError,
  delay,
  finalize,
...
export class ProjectsService {
  ...
  getProjectsByUser(): Observable<boolean> {
    return this.httpClient
      .get<ProjectsResponse[]>(
        `http://localhost:5263/api/proyectos/${
          this.authService.userData()?.id_usuario
        }`
      )
      .pipe(
        map((projects) => {
  ...
  postProject(project: any): Observable<boolean> {
    ...
    return this.httpClient
      .post<ProjectsResponse[]>('http://localhost:5263/api/proyectos', project)
      .pipe(
        delay(5000),
        map(() => {
  ...
  postPhaseProject(id: number, phaseData: any) {
    ...
    return this.httpClient
      .post(`http://localhost:5263/api/proyectos/hitoproyecto/${id}`, phaseData)
      .pipe(
        delay(5000),
        map(() => {
          this.notificationStatusService.statusMessage.set(true);
  ...
  putPhaseProject(phaseData: any, id: number) {
    return this.httpClient
      .put(
        `http://localhost:5263/api/proyectos/hitoproyecto/hito/${id}`,
        phaseData
      )
      .pipe(
        map(() => {
